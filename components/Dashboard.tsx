...
export const Dashboard: React.FC<DashboardProps> = ({ onProcessClick, userProfile }) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // 1. Load Processes (Agora inclui o nome do analista)
  ...
  useEffect(() => {
    // Carrega dados ao montar ou mudar perfil
    loadData();
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'processes' }, () => loadData())
                                ...
                                <td className="px-4 py-3">
                                    <div className="flex items-center gap-2">
                                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${
                                            p.status === 'PROTOCOLADO' ? 'bg-slate-800 text-slate-400 border-slate-700' :
