...
import { ProcessService, ProjectParameterService, LegislationService } from '../services/supabase';
...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ mode, messages: initialMessages, setMessages: setGlobalMessages, processId, onOpenAudit }) => {
  ...
  const handleQuickActionWrapper = async (specialtyId: string, label: string) => {
    ...
    try {
      ...
      try {
        ...
        const activeLaws = laws.filter((l: any) => l.extracted_text && l.extracted_text.length > 50);
        if (activeLaws.length > 0) {
          legislationContext = `\n[LEGISLAÇÃO MUNICIPAL VIGENTE]\n${activeLaws.map((l: any) => `>>> LEI: ${l.name} (${l.category})\n${l.extracted_text.substring(0, 50000)}...`).join('\n\n')}\n`;
        } else {
          legislationContext = `\n[LEGISLAÇÃO]\n(Atenção: O sistema não detectou o conteúdo textual das leis. A análise será baseada em conhecimento geral).\n`;
        }
      } catch (e) { console.warn("Erro ao carregar leis:", e); }

      const rawFiles = await Promise.all(targetDocs.map(async d => {
        // TURBO: Se já tem texto no banco, não baixa o PDF
        if (d.extracted_text && d.extracted_text.length > 10) {
          return {
            data: '', mimeType: 'application/pdf', name: d.name, type: d.file_type, id: d.id, error: false,
            cachedText: d.extracted_text
          };
        }
      ...
      for (const file of rawFiles) {
        ...
        // Se veio do cache (Banco de Dados)
        if ((file as any).cachedText) {
          extractedTextContext += `\n--- DOC (CACHE): ${file.name} ---\n${(file as any).cachedText}\n`;
          continue;
        }
...
[DADOS DO PROCESSO]
${extractedTextContext.substring(0, 1500000)} 
                ...
                <div className={`px-5 py-3.5 rounded-2xl shadow-sm ${msg.role === Role.USER ? 'bg-slate-800 text-slate-100 rounded-tr-sm' : 'bg-slate-900/50 border border-slate-800 text-slate-200 rounded-tl-sm'}`}>
                  {msg.role === Role.MODEL ? (
                    <div className="prose prose-invert prose-sm max-w-none">
                      ...
                      {/* AUDIT COCKPIT TRIGGER - RESTORED TO POPUP */}
                      {onOpenAudit && (
                        msg.text.includes('LAUDO TÉCNICO') ||
                        msg.text.includes('QUADRO TÉCNICO') ||
                        msg.text.includes('MÓDULO') ||
                        msg.text.includes('CHECKLIST') ||
                        msg.text.includes('AUDITORIA')
                      ) && (
                          <div className="mt-4 pt-4 border-t border-slate-700/50">
                            <button
                              onClick={() => onOpenAudit(msg.text)}
                              className="flex items-center gap-2 px-6 py-4 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white rounded-xl shadow-xl shadow-emerald-900/20 transform hover:scale-[1.02] transition-all font-bold text-sm w-full justify-center md:w-auto ring-1 ring-white/10"
                            >
                              <Scale size={20} className="text-emerald-200" />
                              <div className="text-left">
                                <span className="block text-[10px] font-normal text-emerald-200 uppercase tracking-wider">Ação Recomendada</span>
                                <span className="block text-sm">Abrir Checklist de Auditoria</span>
                              </div>
                            </button>
                          </div>
                        )}
