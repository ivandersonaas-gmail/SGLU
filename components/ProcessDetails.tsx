...
import { X, FileText, Upload, Loader2, Clock, Eye, Printer, ArrowRightLeft, Gavel, Save, AlertTriangle, Scale, ShieldCheck, Trash2, Lock, Edit2, ArrowRight, ArrowLeft, CheckCircle2, MinusCircle, XCircle, Calculator, Briefcase, Ruler, Calendar, SendHorizontal, Undo2, PenTool, Link, Map, Search } from 'lucide-react';
              ...
              {activeTab === 'AUDIT' && (
                  <div className="space-y-6 h-full flex flex-col">
                      ...
                      {!auditParams?.ai_validation_summary && wizardStep === 1 && (
                          <div className="text-center py-8"><ShieldCheck className="mx-auto text-slate-600 mb-4" size={48} /><h3 className="text-lg font-bold text-slate-300 mb-2">Auditoria Não Iniciada</h3>{canEdit ? (<button onClick={handleStartManualAudit} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-lg text-sm font-medium flex items-center gap-2 mx-auto"><Edit2 size={16} /> Iniciar Manualmente</button>) : (<div className="mt-2 bg-amber-900/20 text-amber-400 p-3 rounded border border-amber-800/50 text-sm inline-block">Aguardando início da auditoria pelo Licenciamento.</div>)}</div>
                      )}
